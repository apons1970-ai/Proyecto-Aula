...
export function bindActivities(container,activities){
  container.querySelectorAll('.activity[data-type="choice"]').forEach(el=>{
    ...
    el.querySelector('.check').addEventListener('click',()=>{
      let score=0;a.questions.forEach((q,i)=>{const qel=el.querySelector(`.question[data-q="${i}"]`);const selected=Number(qel.dataset.selected);const res=qel.querySelector('.result');if(selected===q.answer){score++;res.className='result ok';res.textContent='✓ Richtig';}else{res.className='result bad';res.textContent=`✗ ${q.feedback||'Versuche es noch einmal.'}`;}});el.querySelector('.score').textContent=`${score} / ${a.questions.length} correctas`;
    });
  });
  container.querySelectorAll('.activity[data-type="gaps"]').forEach(el=>{
    const a=activities[Number(el.dataset.idx)];
    el.querySelector('.check').addEventListener('click',()=>{let score=0;a.items.forEach((it,i)=>{const row=el.querySelector(`.gap-row[data-q="${i}"]`);const ok=it.answers.map(normalize).includes(normalize(row.querySelector('input').value));const res=row.querySelector('.result');if(ok){score++;res.className='result ok';res.textContent='✓';}else{res.className='result bad';res.textContent='✗';}});el.querySelector('.score').textContent=`${score} / ${a.items.length} correctas`;const slot=el.querySelector('.optional-slot');if(slot&&score===a.items.length){slot.innerHTML=`<div class="optional"><b>✓ ¡Muy bien!</b><p>¿Quieres practicar también el significado?</p><button class="secondary meaning-btn">🇩🇪 Con alemán</button><div class="meaning-list" hidden>${a.items.map(x=>`<p><b>${esc(x.text.replace('___',x.answers[0]))}</b><br><span>${esc(x.meaning||'')}</span></p>`).join('')}</div></div>`;slot.querySelector('.meaning-btn').addEventListener('click',()=>slot.querySelector('.meaning-list').toggleAttribute('hidden'));}});
  });
